...
const ChartCard = ({ data }: ChartCardProps) => {
  const { type, completed, inprogress, delayed } = data;
  ...
  return (
    <Card className="max-w-[300px] sm:max-w-[380px] shadow-md shadow-slate-100 border-2 border-slate-100 rounded-2xl">
      <CardHeader>
        <CardTitle className="base-semibold sm:h3-bold text-slate-700">
          {type}
        </CardTitle>
      </CardHeader>
      <CardContent className=" flex flex-col md:relative border-slate-50/0">
        ...
        <div className="flex flex-col gap-2 text-[12px] sm:text-sm">
          <h4 className="small-semibold sm:base-semibold text-slate-600">
            {total} {type} in total
          </h4>
